...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        weight = np.random.normal(0,0.1, (input_D, output_D))
        bias = np.random.normal(0, 0.1, output_D)
        
        self.params.update( {'W' : weight} )
        self.params.update( {'b' : bias} )
        ###############################################################################################

        self.gradient = dict()
        ###############################################################################################
        # TODO: Initialize gradients with zeros
        w_grad = np.ones((input_D, output_D))
        b_grad = np.ones(input_D)
        
        self.gradient.update( {'W' : w_grad} )
        self.gradient.update( {'b' : b_grad} )
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(grad.T,np.ones(len(X)))
...
class relu:
    ...
    def forward(self, X):
        ...
        for i in range(len(self.mask)):
        	if max(0, self.mask[i]) == 0:
        		self.mask[i] = 0
        	else:
        		self.mask[i] = 1


        self.mask = self.mask.reshape(shape)
...
class tanh:
    ...
    def forward(self, X):
        ...
        forward_output = X.reshape(shape)
        ################################################################################
        return forward_output
